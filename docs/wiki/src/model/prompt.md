...
![프롬포트 아키텍처](../assets/images/model/prompt_architecture.png)
...
## 9. Parse 프롬프트 (JsonOutputParser 도입)

자연어 요청을 JSON으로 구조화할 때, 출력의 안정성을 보장하기 위해 LangChain의 **`JsonOutputParser`**와 Pydantic 모델을 결합했습니다.

### 9.1 구조화 전략
단순한 텍스트 출력이 아닌, 프롬프트에 `{format_instructions}`를 주입하여 모델이 **엄격한 JSON 스키마**를 따르도록 강제합니다. 이를 통해 파이프라인 후속 단계에서 발생할 수 있는 파싱 에러를 근본적으로 차단합니다.

### 9.2 Regex 하이브리드 전략
LLM 파서 호출 전에 정규표현식 기반 파서(`parse_user_request_regex`)가 먼저 동작하여 비용과 지연 시간을 최적화합니다. Regex가 모든 필드를 채우지 못한 경우에만 `JsonOutputParser` 체인이 구동됩니다.
