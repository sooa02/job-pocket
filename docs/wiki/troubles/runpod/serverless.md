...
4. [pip 미인식으로 인한 Accelerate 설치 실패 — `device_map` 우회](#4-pip-미인식으로-인한-accelerate-설치-실패--device_map-우회)
...
8. [torch packing 실패 — `--no-deps` 우회 시에도 deploy 차단](#8-torch-packing-실패----no-deps-우회-시에도-deploy-차단)
...
- Endpoint `dependencies` 에 `sentencepiece` 또는 `tiktoken` 이 누락된 경우
...
Endpoint 정의의 `dependencies` 에 패키지를 추가한다.
...
```bash
runpod flash build && runpod flash deploy
```

---

## 4. pip 미인식으로 인한 Accelerate 설치 실패 — `device_map` 우회
...
- `dependencies` 에 `accelerate` 를 추가했으나 실제로 설치되지 않은 경우
- `device_map="auto"` 또는 `low_cpu_mem_usage=True` 옵션을 사용하는 경우

### 원인

**근본 원인은 RunPod Serverless 플랫폼의 pip 미인식 문제**다.  
Flash 런타임 내부에서 `pip` 가 정상적으로 동작하지 않아 `dependencies` 에 명시한 패키지가 실제로 설치되지 않는다.  
이는 RunPod 플랫폼 자체의 이슈로, 직접적인 해결이 불가능하다.
...
`accelerate` 의존 자체를 제거하는 방향으로 전환한다.  
`device_map="auto"` 대신 `.to(device)` 를 사용해 accelerate 없이 GPU에 모델을 올린다.
...
> **참고:** pip 미인식은 RunPod Serverless 플랫폼 자체 이슈로 현재 미해결 상태.  
> 근본 해결보다 accelerate 의존을 코드에서 제거하는 방향을 권장한다.
...
```python
# ExaoneInput 기본값 조정
class ExaoneInput(TypedDict, total=False):
    messages: list[ChatMessage]
    temperature: float
    max_new_tokens: int  # 요청 시 최대 1024 이하로 제한
```
...
```python
# 올바른 경로 예시
MODEL_PATH = "/runpod-volume/models/EXAONE-3.5-7.8B-Instruct"
```
...
@endpoint(
    name="exaone-main",
    gpu=GPU.ADA_48_PRO,
    volume=network_volume,          # ← Volume 객체 연결 여부 확인
...
from runpod import endpoint, GPU
...
## 8. torch packing 실패 — `--no-deps` 우회 시에도 deploy 차단
...
`runpod flash build` 에서 `torch` packing이 실패하며, `--no-deps` 옵션으로 우회해도 deploy 단계에서 차단된다.

### 재현 조건

- `dependencies` 에 `torch` 를 직접 포함한 경우
- `--no-deps` 옵션으로 torch를 packing 대상에서 제외했으나 deploy 시 의존성 검증에서 실패한 경우
- CPU 전용 torch wheel 또는 플랫폼 불일치 wheel을 사용한 경우

### 원인

`torch` 는 패키지 크기가 매우 크고(~2GB) 플랫폼(CUDA 버전)에 종속적이다.  
Flash의 packing 단계는 wheel을 번들링하는 방식인데, torch는 이 과정에서 크기 제한 또는 플랫폼 태그 불일치로 실패한다.  
`--no-deps` 로 packing을 건너뛰어도 deploy 시 런타임 의존성 검증이 별도로 수행되어 차단된다.

### 해결 절차

`torch` 는 packing 대상에서 제외하고, **RunPod 기본 이미지에 사전 설치된 torch를 사용**한다.
...
RunPod GPU 기본 이미지에는 CUDA 호환 torch가 사전 설치되어 있으므로 별도 packing 없이 사용 가능하다.
...
- `torch`, `torchvision`, `torchaudio` 는 `dependencies` 에 명시하지 않는다.
- CUDA 버전 호환성이 필요한 패키지는 RunPod 기본 이미지 제공 버전을 우선 사용한다.
...
**② handler 내부 스키마를 TypedDict로 전환 (Pydantic 제거)**

```python
# ❌ Pydantic 사용 — Flash 환경에서 불안정
from pydantic import BaseModel
class ExaoneRequest(BaseModel):
    messages: list
    temperature: float

# ✅ TypedDict 사용 — Flash 환경에서 안정적
...
- Flash handler 내부는 표준 라이브러리 + `TypedDict` 만 사용하고 Pydantic 의존을 금지한다.
...
| `dependencies` 완전성 | `transformers`, `sentencepiece` 포함 여부 (`torch`, `accelerate` 제외) |
| `torch` 미명시 | `dependencies` 에 `torch` 가 없는지 확인 (기본 이미지 사용) |
| `device_map="auto"` 미사용 | `.to(device)` 방식 사용 여부 확인 (accelerate 의존 제거) |
| `@Endpoint` 직접 호출 금지 | FastAPI에서 HTTP 호출 방식 사용 여부 확인 |
| handler 스키마 TypedDict 사용 | Pydantic 대신 `TypedDict` 사용 여부 확인 |
| `RUNPOD_API_KEY` 환경변수 | `echo $RUNPOD_API_KEY` 또는 `.env` 확인 |
| 모델 경로 일치 | 코드 상수 vs Volume 실제 경로 일치 여부 |
| `max_new_tokens` 상한 | 1024 이하 설정 여부 |
