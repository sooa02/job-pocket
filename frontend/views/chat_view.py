...
def render_assistant_message(content: str, message_index: int):
    ...
    if is_result_message:
        ...
        st.caption(caption)
        with st.expander("📄 복사하기"): st.code(body_text, language="plaintext")
        st.divider()
        ...
        if st.session_state[feedback_key] is None:
            ...
            st.markdown(f"<div class='center-helper'>✓ 평가 완료: {fb}</div>", unsafe_allow_html=True)
            if fb == "👎": st.info("어떤 부분이 아쉬웠는지 말씀해 주세요. 예: 첫 문장 구체화, 더 담백하게, 마지막 문단 수정")
    else:
        st.markdown(content)
...
def process_prompt(prompt: str, user_email: str):
    ...
    with st.chat_message("assistant", avatar=AI_AVATAR):
        result_box = st.empty()
        try:
            response = generate_response_with_progress(prompt=prompt, user_info=st.session_state.user_info, selected_model=st.session_state.selected_model)
...
def chat_view():
    ...
    if "show_welcome" not in st.session_state: st.session_state.show_welcome = not bool(st.session_state.messages)
        ...
        with col_btn:
            if st.button("🚀 대화 시작하기", use_container_width=True, type="primary"):
                ...
                if not st.session_state.messages:
                    greeting = "안녕하세요! 지원하시려는 **회사와 직무**, 그리고 **자기소개서 문항**을 편하게 남겨주세요. 초안이 생성된 뒤에는 문장 수정, 톤 변경, 글자 수 조정도 이어서 도와드릴게요. 😊"
    ...
    for i, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"], avatar=USER_AVATAR if message["role"] == "user" else AI_AVATAR):
            if message["role"] == "assistant": render_assistant_message(message["content"], i)
    ...
    if prompt := st.chat_input(get_chat_input_placeholder()): process_prompt(prompt, user_email)
